...
class UserGroupManagerController(QObject):
    ...
    changed_group_members_signal: Signal = Signal(str, int)
    created_group_signal: Signal = Signal(str, int)
    deleted_group_signal: Signal = Signal(str, int)
    ...
    def fetch_users_and_groups(self, for_refresh: bool) -> None:
        """Fetch user and group information from /etc/passwd and /etc/group with the built in Python libs."""
        users_raw = getpwall()
        groups_raw = getgrall()
        processed_groups: list = [
            GroupInfo(group.gr_name, group.gr_gid, group.gr_mem) for group in groups_raw
        ]
        processed_users = [
    ...
    def user_add(self, username: str, fullname: str,  homedir: str, homedirtype: Literal["none", "auto", "selectexisting"], password: str, shell: str, groups: list[str]) -> None:
        ...
        match homedirtype:
            case "selectexisting":
                command.extend(["-d", homedir])
            case "auto":
                command.append("-m")
        command.extend(["-s", shell])
        if fullname != "":
            command.extend(["-c", fullname])
        if groups:
            command.extend(["-G", ",".join(groups)])
        command.append(username)

        self.useradd_worker.start_process("pkexec", command)
    ...
    def change_shell(self, username: str, shell: str) -> None:
        ...
        self.usermod_shell_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
    ...
    def user_delete(self, username: str, homedir_delete: bool) -> None:
        ...
        self.userdel_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
    ...
    def change_full_name(self, username: str, full_name: str) -> None:
        ...
        self.usermod_gecos_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
    ...
    def change_homedir(self, username: str, homedir: str) -> None:
        ...
        self.usermod_homedir_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
    ...
    def modify_group_users(self, groupname: str, users: list[str, bool]) -> None:
        ...
        for user in users:
            if user[1]:
                user_remove.append(user[0])
            elif not user[1]:
                user_add.append(user[0])
        ...
        self.group_manager_worker: QProcessHandler = QProcessHandler()
        self.group_manager_worker.finished_with_exit_code.connect(lambda _, exit_code: self.changed_group_members_signal.emit(groupname, exit_code))
        self.group_manager_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
        self.group_manager_worker.start_process("pkexec",
            ["sh", "-c", f"{' && '.join(command_queue)}"]
        )

    def group_add(self, groupname: str, is_system_group: bool, users: list[str]) -> None:
        """Runs the groupadd command to create a new group."""
        command: list[str] = ["groupadd"]
        if is_system_group:
            command.append("-r")
        command.append("-U")
        command.append(",".join(users))

        self.groupadd_worker: QProcessHandler = QProcessHandler()
        self.groupadd_worker.finished_with_exit_code.connect(lambda _, exit_code: self.created_group_signal.emit(groupname, exit_code))
        self.groupadd_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
        self.groupadd_worker.start_process("pkexec", command)

    def group_del(self, groupname: str) -> None:
        """Runs groupdel to delete the given group."""
        self.groupdel_worker: QProcessHandler = QProcessHandler()
        self.groupdel_worker.finished_with_exit_code.connect(lambda _, exit_code: self.deleted_group_signal.emit(groupname, exit_code))
        self.groupdel_worker.finished.connect(lambda: self.changed_user_group_properties_signal.emit())
        self.groupdel_worker.start_process("pkexec", ["groupdel", groupname])
