...
class ServiceManagerController(QObject):
    ...
    systemctl_operation_done: Signal = Signal(bool, str)
    ...
    def __init__(self) -> None:
        ...
        self.systemctl_operation_done.connect(lambda is_user_unit, type: self.list_units(True, is_user_unit, type))
    ...
    def _list_units_internal(self, unit_type: Literal["service", "timer", "socket"], is_user_level: bool, ) -> list:
        """Fetches systemd services off the systemd API."""
        bus = self.user_bus if is_user_level else self.system_bus
        systemd_bus = self.systemd_user_bus if is_user_level else self.systemd_system_bus
    ...
    def list_units(self, is_for_refresh: bool, is_user_level: bool, unit_type: Literal["service", "timer", "socket"]) -> None:
        """Returns unit lists for their respective categories."""
        self._acquire_systemctl()
        units: list[SystemdServiceInfo] = self._list_units_internal(unit_type, is_user_level)
        if is_for_refresh:
            match unit_type:
                case "service":
                        self.services_fetched_for_refresh.emit(is_user_level, units)
                case "timer":
                        self.timers_fetched_for_refresh.emit(is_user_level, units)
                case "socket":
                        self.sockets_fetched_for_refresh.emit(is_user_level, units)
        else:
            match unit_type:
                case "service":
                        self.services_fetched_for_init.emit(is_user_level, units)
                case "timer":
                        self.timers_fetched_for_init.emit(is_user_level, units)
                case "socket":
                        self.sockets_fetched_for_init.emit(is_user_level, units)
    ...
    def call_systemctl(self, is_user_unit: bool, unit: str,
        ...
        self.systemctl_worker.finished.connect(lambda: self.systemctl_operation_done.emit(is_user_unit, unit_type))
